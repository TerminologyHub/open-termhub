...
- `READ_ONLY`: When set to `true`, rejects HTTP APIs that add, alter, or remove content (403) and disables startup/cron syndication. Read APIs (including FHIR `$` operations and `POST /concept/bulk`) remain available at runtime. Swagger/OpenAPI for REST and FHIR hide DELETE, PUT, PATCH, and POST operations. Default: false.
