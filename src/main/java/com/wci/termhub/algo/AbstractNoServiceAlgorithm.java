...
public abstract class AbstractNoServiceAlgorithm implements NoServiceAlgorithm {
	 ...
	 * @throws Exception the exception
	 */
	public void fireAdjustedProgressEvent(final int pct, final int step, final int steps, final String note)
			throws Exception {
		final ProgressEvent pe = new ProgressEvent(this, pct, pct, note);
		for (int i = 0; i < listeners.size(); i++) {
			listeners.get(i).updateProgress(pe);
	 ...
	 * @throws Exception the exception
	 */
	public void checkRequiredProperties(final String[] required, final Properties p) throws Exception {
		if (p == null) {
