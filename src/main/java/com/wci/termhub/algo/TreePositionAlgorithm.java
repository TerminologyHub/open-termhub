...
//	private long batchSize = 0;
	...
	public void compute() throws Exception {
		...
		final Terminology term = TerminologyUtility.getTerminology(searchService, getTerminology(), getPublisher(),
				getVersion());
		if (term == null) {
			throw new Exception(
					"Unable to find terminology = " + getTerminology() + ", " + getPublisher() + ", " + getVersion());
		}
...
//					ModelUtility.asList("id", "from.code", "to.code", "additionalType"), ConceptRelationship.class,
//					searchAfter, prefixes);
			...
			for (final ConceptRelationship rel : innerList.getItems()) {
				if (additionalTypeMap.containsKey(rel.getFrom().getCode())
					...
					// being involved in multiple trees each with their own relationship type
					// Since ATC mixes RELAs in the same tree, we would then need terminology-level
					// metadata about what kind of "case" we are dealing with so this algorithm
					// could respond appropriately
					throw new Exception("Unexpected condition: same code, multiple additional relationship types = "
		...
		for (final String rootCode : rootCodes) {
			...
			computeTreePositions(rootCode, "", parChd, result, startDate, rootCodes.size() > 1, additionalTypeMap);
			if (!result.isValid()) {
		...
		term.setTreePositionCt(TerminologyUtility.countTreePositions(searchService, term.getAbbreviation(),
	...
	public Set<String> computeTreePositions(final String code, final String ancestorPath,
			final Map<String, Set<String>> parChd, final ValidationResult validationResult, final Date startDate,
			final boolean multipleRoots, final Map<String, String> additionalTypeMap) throws Exception {
		...
		if (parChd.containsKey(code)) {
			...
			for (final String childConceptCode : parChd.get(code)) {
				...
				final Set<String> desc = computeTreePositions(childConceptCode, conceptPath, parChd, validationResult,
						startDate, multipleRoots, additionalTypeMap);
