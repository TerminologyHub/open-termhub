...
public class CodeSystemProviderR4 implements IResourceProvider {
	...
	public CodeSystem getCodeSystem(final HttpServletRequest request, final ServletRequestDetails details,
		...
		try {
			...
			throw FhirUtilityR4.exception("Code system not found = " + (id == null ? "null" : id.getIdPart()),
			...
			throw FhirUtilityR4.exception("Failed to load code system", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	...
	public Bundle findCodeSystems(final HttpServletRequest request, final ServletRequestDetails details,
		...
		try {
			...
			logger.error("Unexpected FHIR error", e);
			throw FhirUtilityR4.exception("Failed to find code systems", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	...
	public Parameters lookupImplicit(final HttpServletRequest request, final HttpServletResponse response,
		...
		if (request.getMethod().equals("POST")) {
			throw FhirUtilityR4.exception("POST method not supported for $lookup", IssueType.NOTSUPPORTED, 405);
		}

		try {
			FhirUtilityR4.mutuallyExclusive("code", code, "coding", coding);

			final Terminology terminology = FhirUtilityR4.getTerminology(searchService, null, code, "system", system,
					version, coding);
			...
			logger.error("Unexpected FHIR error", e);
			throw FhirUtilityR4.exception("Failed to lookup code", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	...
	public Parameters lookupInstance(final HttpServletRequest request, final HttpServletResponse response,
		...
		try {
			...
			final Terminology terminology = FhirUtilityR4.getTerminology(searchService, id, null, "system", null, null,
					null);
	...
	public Parameters validateCodeImplicit(final HttpServletRequest request, final HttpServletResponse response,
		...
		// Reject post
		if (request.getMethod().equals("POST")) {
			throw FhirUtilityR4.exception("POST method not supported for $validate-code", IssueType.NOTSUPPORTED, 405);
		}
		...
		try {
			...
			final Terminology terminology = FhirUtilityR4.getTerminology(searchService, null, code, "url", url, version,
					coding);
			...
			logger.error("Unexpected FHIR error", e);
			throw FhirUtilityR4.exception("Failed to validate code", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	...
	public Parameters validateCodeInstance(final HttpServletRequest request, final HttpServletResponse response,
			...
			@OperationParam(name = "display", min = 0, max = 1, typeName = "string") final StringType display,
			@OperationParam(name = "coding", min = 0, max = 1, typeName = "Coding") final Coding coding,
			@OperationParam(name = "codeableConcept", min = 0, max = 1, typeName = "CodeableConcept") final CodeableConcept codeableConcept)
			throws Exception {
		...
		try {
			...
			final Terminology terminology = FhirUtilityR4.getTerminology(searchService, id, code, "url", url, version,
					coding);
	...
	public Parameters subsumesImplicit(final HttpServletRequest request, final HttpServletResponse response,
		...
		// Reject post
		if (request.getMethod().equals("POST")) {
			throw FhirUtilityR4.exception("POST method not supported for $subsumes", IssueType.NOTSUPPORTED, 405);
		}
		...
		try {
			...
			logger.error("Unexpected FHIR error", e);
			throw FhirUtilityR4.exception("Failed to check if A subsumes B", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	...
	private Parameters subsumesHelper(final AuthContext context, final IdType id, final UriType system,
		...
		final Terminology terminologyA = FhirUtilityR4.getTerminology(searchService, id, codeAParam, "system", system,
				version, codingA);
		final Terminology terminologyB = FhirUtilityR4.getTerminology(searchService, id, codeBParam, "system", system,
				version, codingB);

		if (terminologyA == null && terminologyB == null) {
			throw FhirUtilityR4.exception("Unable to determine code system for $subsumes operation.", IssueType.INVALID,
					400);
		}
		...
		if (terminologyA == null) {
			...
			throw FhirUtilityR4.exception("Incompatable code system A/B for $subsumes operation.", IssueType.INVALID,
					400);
		}
		...
		if (conceptA == null) {
			throw FhirUtilityR4.exception(String.format(
