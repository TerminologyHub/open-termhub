...
public class ConceptMapProviderR4 implements IResourceProvider {
	...
	public ConceptMap getConceptMap(final HttpServletRequest request, final ServletRequestDetails details,
		...
		try {
			...
			throw FhirUtilityR4.exception("Concept map not found = " + (id == null ? "null" : id.getIdPart()),
			...
			throw FhirUtilityR4.exception("Failed to load concept map", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	...
	public Bundle findConceptMaps(final HttpServletRequest request, final ServletRequestDetails details,
		...
		try {
			...
			for (final ConceptMap cm : candidates) {
				...
				if (identifier != null && !FhirUtility.compareString(identifier, cm.getIdentifier().getValue())) {
				...
				if (sourceSystem != null && !cm.getSourceUriType().getValue().startsWith(sourceSystem.getValue())) {
					logger.info("  SKIP sourceSystem mismatch = " + sourceSystem.getValue() + ", "
							+ cm.getSourceUriType().getValue());
					continue;
				}
				if (targetSystem != null && !cm.getTargetUriType().getValue().startsWith(targetSystem.getValue())) {
			...
			logger.error("Unexpected FHIR error", e);
			throw FhirUtilityR4.exception("Failed to find concept maps", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	...
	public Parameters translateInstance(final HttpServletRequest request, final HttpServletResponse response,
			...
			@OperationParam(name = "code") final CodeType code, @OperationParam(name = "system") final UriType system,
		...
		if (request.getMethod().equals("POST")) {
			throw FhirUtilityR4.exception("POST method not supported for $translate", IssueType.NOTSUPPORTED, 405);
		}

		FhirUtilityR4.requireExactlyOneOf("code", code, "coding", coding, "codeableConcept", codeableConcept);
		...
		try {
			...
			for (final ConceptMap cm : candidates) {
				...
				if (source != null && !source.getValue().equals(cm.getSourceUriType().getValue())) {
					logger.info(
							"  SKIP source mismatch = " + source.getValue() + ", " + cm.getSourceUriType().getValue());
					continue;
				}
				if (target != null && !target.getValue().equals(cm.getTargetUriType().getValue())) {
					logger.info(
							"  SKIP target mismatch = " + target.getValue() + ", " + cm.getTargetUriType().getValue());
					continue;
				}
				if (targetSystem != null && !cm.getTargetUriType().getValue().startsWith(targetSystem.getValue())) {
					logger.info("  SKIP targetSystem mismatch = " + targetSystem.getValue() + ", "
			...
			logger.error("Unexpected FHIR error", e);
			throw FhirUtilityR4.exception("Failed to translate concept map", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	 ...
	 * @throws Exception the exception
	 */
	private Parameters translateHelper(final List<ConceptMap> maps, final Terminology terminology, final String code,
