  ...
  public Questionnaire getQuestionnaire(final HttpServletRequest request,
    ...
    try {
      ...
      if (byId != null && isQuestionnaireConcept(byId)) {
        ...
        return QuestionnaireGetCache.getOrLoadR4(cacheKey, () -> {
          final Questionnaire questionnaire =
              FhirUtilityR4.toR4Questionnaire(byId, searchService, resolvedTerminology);
          FhirUtilityR4.populateQuestionnaire(questionnaire, searchService, resolvedTerminology);
      ...
      return QuestionnaireGetCache.getOrLoadR4(cacheKey, () -> {
        ...
        if (concept == null) {
          throw FhirUtilityR4.exception("Questionnaire not found = " + idPart, IssueType.NOTFOUND,
              HttpServletResponse.SC_NOT_FOUND);
        }

        if (!isQuestionnaireConcept(concept)) {
          throw FhirUtilityR4.exception("Concept " + idPart + " is not a questionnaire",
  ...
  private Terminology requireLoincTerminology(final StringParam version)
    ...
    if (terminology == null) {
      if (versionValue != null && !versionValue.isBlank()) {
        throw FhirUtilityR4.exception("LOINC terminology version not found = " + versionValue,
            IssueType.NOTFOUND, HttpServletResponse.SC_NOT_FOUND);
      }
