 ...
 */
@Component
public class SystemTransactionProviderR4 {

  /** The logger. */
  private static Logger logger = LoggerFactory.getLogger(SystemTransactionProviderR4.class);
    ...
    for (final Bundle.BundleEntryComponent in : bundle.getEntry()) {
      ...
      try {
        if (in.getResource() == null) {
          throw FhirUtilityR4.exception("Missing entry.resource", IssueType.INVALID,
        ...
        if (!("CodeSystem".equals(resourceType) || "ValueSet".equals(resourceType)
          ...
          throw FhirUtilityR4.exception("Unsupported resource type in bundle: " + resourceType,
              IssueType.INVALID, HttpServletResponse.SC_BAD_REQUEST);
        }

        if (verb != Bundle.HTTPVerb.POST && verb != Bundle.HTTPVerb.PUT) {
          throw FhirUtilityR4.exception("Unsupported method: " + verb, IssueType.NOTSUPPORTED,
              HttpServletResponse.SC_BAD_REQUEST);
        }
