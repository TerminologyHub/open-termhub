...
public class CodeSystemProviderR5 implements IResourceProvider {
  ...
  private Parameters lookupHelper(final Terminology terminology, final String code,
    ...
    // LOINC toR5 does not use conceptNameMap; skip the expensive full-concept scan
    ...
    final Parameters result = FhirUtilityR5.toR5(FhirUtilityR5.toR5(terminology), concept,
        propertySet, displayMap, relationships,
        children == null ? null : children.stream().map(r -> r.getFrom()).toList(),
        conceptNameMap, searchService, regenstriefMode);
    CodeSystemLookupCache.putR5(cacheKey, result);
    return result;
  }
  ...
  private Parameters subsumesHelper(final IdType id, final UriType system, final StringType version,
    ...
    final Terminology terminologyA = FhirUtilityR5.getTerminology(searchService, id, codeAParam,
        "system", system, version, codingA);
    final Terminology terminologyB = FhirUtilityR5.getTerminology(searchService, id, codeBParam,
