...
public class CodeSystemProviderR5 implements IResourceProvider {
	 ...
	 */
	@Read()
	public CodeSystem getCodeSystem(final HttpServletRequest request, final ServletRequestDetails details,
		...
		try {
			...
			throw FhirUtilityR5.exception("Code system not found = " + (id == null ? "null" : id.getIdPart()),
	 ...
	 */
	@Search
	public Bundle findCodeSystems(final HttpServletRequest request, final ServletRequestDetails details,
			@OptionalParam(name = "_id") final TokenParam id, @OptionalParam(name = "code") final TokenParam code,
			...
			@OptionalParam(name = "title") final StringParam title, @OptionalParam(name = "url") final UriParam url,
		...
		try {
			...
			for (final Terminology terminology : FhirUtility.lookupTerminologies(searchService)) {
				...
				if (code != null) {
					mapsetsMatchingCodes.addAll(searchService
							.find(new SearchParameters("to.code:" + StringUtility.escapeQuery(code.getValue()), null,
									1000, null, null), Concept.class, null)
			...
			logger.error("Unexpected FHIR error", e);
			throw FhirUtilityR5.exception("Failed to find code systems", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	...
	public Parameters lookupImplicit(final HttpServletRequest request, final HttpServletResponse response,
			...
			@OperationParam(name = "version", min = 0, max = 1, typeName = "string") final StringType version,
			@OperationParam(name = "coding", min = 0, max = 1, typeName = "Coding") final Coding coding,
			@OperationParam(name = "displayLanguage", min = 0, max = 1, typeName = "code") final CodeType displayLanguage,
			@OperationParam(name = "property", min = 0, max = OperationParam.MAX_UNLIMITED, typeName = "code") final Set<CodeType> properties)
			throws Exception {
		final AuthContext context = FhirUtility.authorize(request);

		// Reject post
		if (request.getMethod().equals("POST")) {
			throw FhirUtilityR5.exception("POST method not supported for $lookup", IssueType.NOTSUPPORTED, 405);
		}
		...
		try {
			...
			final Terminology terminology = FhirUtilityR5.getTerminology(searchService, null, code, "system", system,
					version, coding);
			...
			logger.error("Unexpected FHIR error", e);
			throw FhirUtilityR5.exception("Failed to lookup code", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	 ...
	 */
	@Operation(name = JpaConstants.OPERATION_LOOKUP, idempotent = true)
	public Parameters lookupInstance(final HttpServletRequest request, final HttpServletResponse response,
		...
		try {
			...
			final Terminology terminology = FhirUtilityR5.getTerminology(searchService, id, null, null, null, null,
					null);
	...
	public Parameters validateCodeImplicit(final HttpServletRequest request, final HttpServletResponse response,
			final ServletRequestDetails details,
			@OperationParam(name = "url", min = 0, max = 1, typeName = "uri") final UriType url,
			@OperationParam(name = "code", min = 0, max = 1, typeName = "code") final CodeType code,
			@OperationParam(name = "version", min = 0, max = 1, typeName = "string") final StringType version,
			@OperationParam(name = "display", min = 0, max = 1, typeName = "string") final StringType display,
			@OperationParam(name = "coding", min = 0, max = 1, typeName = "Coding") final Coding coding)
			throws Exception {
		...
		// Reject post
		if (request.getMethod().equals("POST")) {
			throw FhirUtilityR5.exception("POST method not supported for $validate-code", IssueType.NOTSUPPORTED, 405);
		}

		try {

			FhirUtilityR5.mutuallyExclusive("code", code, "coding", coding);
			FhirUtilityR5.mutuallyRequired("display", display, "code", code, "coding", coding);

			final Terminology terminology = FhirUtilityR5.getTerminology(searchService, null, code, "url", url, version,
					coding);
			...
			logger.error("Unexpected FHIR error", e);
			throw FhirUtilityR5.exception("Failed to validate code", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	 ...
	 */
	@Operation(name = JpaConstants.OPERATION_VALIDATE_CODE, idempotent = true)
	public Parameters validateCodeInstance(final HttpServletRequest request, final HttpServletResponse response,
			final ServletRequestDetails details, @IdParam final IdType id,
			@OperationParam(name = "url", min = 0, max = 1, typeName = "uri") final UriType url,
			@OperationParam(name = "version", min = 0, max = 1, typeName = "string") final StringType version,
			@OperationParam(name = "code", min = 0, max = 1, typeName = "code") final CodeType code,
		...
		try {
			...
			final Terminology terminology = FhirUtilityR5.getTerminology(searchService, id, code, "url", url, version,
					coding);
	 ...
	 *
	 * <pre>
	 * https://hl7.org/fhir/R5/codesystem-operation-subsumes.html All parameters
	 * supported. </pre
	 *
	 * @param request  the request
	 * @param response the response
	...
	public Parameters subsumesImplicit(final HttpServletRequest request, final HttpServletResponse response,
			final ServletRequestDetails details,
		...
		// Reject post
		if (request.getMethod().equals("POST")) {
			throw FhirUtilityR5.exception("POST method not supported for $subsumes", IssueType.NOTSUPPORTED, 405);
		}
		...
		try {
			...
			logger.error("Unexpected FHIR error", e);
			throw FhirUtilityR5.exception("Failed to check if A subsumes B", OperationOutcome.IssueType.EXCEPTION, 500);
		}
	}
	 ...
	 */
	@Operation(name = JpaConstants.OPERATION_SUBSUMES, idempotent = true)
	public Parameters subsumesInstance(final HttpServletRequest request, final HttpServletResponse response,
			final ServletRequestDetails details, @IdParam final IdType id,
			@OperationParam(name = "codeA", min = 0, max = 1, typeName = "code") final CodeType codeA,
			@OperationParam(name = "codeB", min = 0, max = 1, typeName = "code") final CodeType codeB,
			@OperationParam(name = "system", min = 0, max = 1, typeName = "uri") final UriType system,
	 ...
	 * @throws Exception the exception
	 */
	private Parameters lookupHelper(final AuthContext context, final Terminology terminology, final String code,
		...
		final List<ConceptRelationship> relationships = searchService.findAll(
				StringUtility.composeQuery("AND", "active:true", "from.code:" + StringUtility.escapeQuery(code)),
				ConceptRelationship.class);
		final List<ConceptRelationship> children = searchService.findAll(StringUtility.composeQuery("AND",
				"active:true", "hierarchical:true", "to.code:" + StringUtility.escapeQuery(code)),
				ConceptRelationship.class);
		...
		return FhirUtilityR5.toR5(FhirUtilityR5.toR5(terminology), concept,
				properties == null ? null : properties.stream().map(c -> c.getValue()).collect(Collectors.toSet()),
				displayMap, relationships,
				children == null ? null : children.stream().map(r -> r.getFrom()).collect(Collectors.toList()));
	}
	 ...
	 * @throws Exception the exception
	 */
	private Parameters validateHelper(final AuthContext context, final Terminology terminology, final String code,
	 ...
	 * @throws Exception the exception
	 */
	private Parameters subsumesHelper(final AuthContext context, final IdType id, final UriType system,
			final StringType version, final CodeType codeAParam, final CodeType codeBParam, final Coding codingA,
		...
		if (id == null && system == null) {
			throw FhirUtilityR5.exception(
					"One of id or system parameters must be supplied for the $subsumes operation.", IssueType.INVALID,
					400);
		}

		FhirUtilityR5.requireExactlyOneOf("codeA", codeAParam, "codingA", codingA);
		FhirUtilityR5.requireExactlyOneOf("codeB", codeBParam, "codingB", codingB);

		final Terminology terminologyA = FhirUtilityR5.getTerminology(searchService, id, codeAParam, "system", system,
				version, codingA);
		final Terminology terminologyB = FhirUtilityR5.getTerminology(searchService, id, codeBParam, "system", system,
				version, codingB);

		if (terminologyA == null && terminologyB == null) {
			throw FhirUtilityR5.exception("Unable to determine code system for $subsumes operation.", IssueType.INVALID,
					400);
		}
		...
		if (terminologyA == null) {
			...
			throw FhirUtilityR5.exception("Incompatable code system A/B for $subsumes operation.", IssueType.INVALID,
					400);
		}

		final String codeA = FhirUtilityR5.getCode(codeAParam, codingA);
		final String codeB = FhirUtilityR5.getCode(codeBParam, codingB);

		final Concept conceptA = TerminologyUtility.getConcept(searchService, terminology, codeA);
		if (conceptA == null) {
			throw FhirUtilityR5.exception(String.format(
					"Code does not exist for code system =" + codeA + "," + terminology.getAttributes().get("fhirUri")),
		...
		if (conceptB == null) {
			throw FhirUtilityR5.exception(String.format(
					"Code does not exist for code system =" + codeB + "," + terminology.getAttributes().get("fhirUri")),
					OperationOutcome.IssueType.INVALID, 400);
		}
		...
		if (codeA.equals(codeB)) {
			return FhirUtilityR5.toR5Subsumes("equivalent", terminology);
		}

		// If A is an ancestor of b
		if (TerminologyUtility.getAncestorCodes(searchService, conceptB).contains(conceptA.getCode())) {
			return FhirUtilityR5.toR5Subsumes("subsumes", terminology);
		} else if (TerminologyUtility.getAncestorCodes(searchService, conceptA).contains(conceptB.getCode())) {
