...
public class ConceptMapProviderR5 implements IResourceProvider {
  ...
  public ConceptMap getConceptMap(final HttpServletRequest request,
    ...
    try {
      ...
      return ConceptMapGetCache.getOrLoadR5(cacheKey, () -> {
        final Mapset mapset = FhirUtility.findMapsetByIdOrCode(searchService, requestedId);
        if (mapset == null) {
      ...
      for (final ConceptMap cm : candidates) {
        ...
        if (sourceSystem != null) {
          ...
          if (cm.getSourceScope() == null || ((UriType) cm.getSourceScope()).getValue() == null
              || !((UriType) cm.getSourceScope()).getValue().startsWith(sourceSystem.getValue())) {
            if (logger.isDebugEnabled()) {
  ...
  public Parameters translateInstance(final HttpServletRequest request,
    ...
    try {
      ...
      final List<Mapset> selected =
          FhirUtility.filterTranslateMapsets(FhirUtility.lookupMapsets(searchService),
              url == null ? null : url.getValue(), id == null ? null : id.getIdPart(),
      ...
      if (!mappings.isEmpty()) {
        for (final Mapping mapping : mappings) {
          ...
          final String fallbackUri =
              reverse ? r5ScopeUri(map.getSourceScope()) : r5ScopeUri(map.getTargetScope());
