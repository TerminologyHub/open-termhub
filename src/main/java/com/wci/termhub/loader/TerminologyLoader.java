...
public final class TerminologyLoader {
  ...
  /** The logger. */
  private static Logger logger = LoggerFactory.getLogger(TerminologyLoader.class);

  /**
   * Instantiates a new terminology loader.
   */
  private TerminologyLoader() {
    // private
  }
  ...
  public static void index(final String fullFileName, final int batchSize, final int limit)
    ...
    final List<Terminology> terminologyBatch = new ArrayList<>(batchSize);
    ...
    try (final BufferedReader br = new BufferedReader(new FileReader(fullFileName))) {
      ...
      luceneDataAccess.createIndex(Terminology.class);

      String line;
      int count = 1;
      while ((line = br.readLine()) != null && (limit == -1 || count < limit)) {

        final JsonNode rootNode = ThreadLocalMapper.get().readTree(line);
        final JsonNode terminologyNode =
            (rootNode.has("_source")) ? rootNode.get("_source") : rootNode;
        final Terminology terminology =
            ModelUtility.fromJson(terminologyNode.toString(), Terminology.class);
        terminologyBatch.add(terminology);

        if (terminologyBatch.size() == batchSize) {
          luceneDataAccess.add(terminologyBatch);
