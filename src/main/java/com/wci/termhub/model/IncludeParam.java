...
public class IncludeParam extends BaseModel {
	...
	public static List<String> getConceptBaseFields() {
		final List<String> fields = ModelUtility.getBaseFields(Concept.class).stream().map(f -> f.getName())
	...
	public boolean hasAnyTrue() {
		return terms || definitions || attributes || children || ancestors || descendants || parents || relationships
				|| inverseRelationships || mapsets || subsets || axioms || treePositions || semanticTypes;
	}
