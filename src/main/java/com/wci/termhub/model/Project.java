...
@Entity
@Table(name = "project")
@Schema(description = "Represents a project used by a user or organization" + " to define a terminology set")
@Document(indexName = "project-v1")
@JsonInclude(Include.NON_EMPTY)
@JsonIgnoreProperties(ignoreUnknown = true)
public class Project extends AbstractHasModified implements HasName, HasLazyInit, Copyable<Project> {
...
//			throw new Exception("uriLabel contains characters other than letters, numbers, _, and -");
//		}
...
//			throw new Exception("Adding an already soft deleted project is not allowed");
//		}
//
//		// Trim certain fields
//		if (getName() != null) {
...
//			throw new Exception("Mismatch between id parameter and id specified in object");
//		}
...
//		if (BooleanUtils.isTrue(getSoftDeleted())) {
//			throw new Exception("Setting soft deleted for project via update is not allowed");
//		}
//
//		// Trim certain fields
//		if (getName() != null) {
	...
	private void validateMapsets() throws Exception {
		...
		for (final TerminologyRef mapset : getMapsets()) {
			...
			if (mapset.getLatest() != null && mapset.getLatest() && mapset.getVersion() != null) {
				throw new Exception(
						"Project unexpectedly has a terminology with both 'version' and 'latest' set = " + mapset);
			}
	...
	public boolean checkTerminology(final Terminology terminology) {
		return terminology != null && getTerminologies().stream()
				.filter(t -> terminology.getAbbreviation().equals(t.getAbbreviation())
						...
						&& (terminology.getVersion().equals(t.getVersion()) || (t.getLatest() != null && t.getLatest()
								&& terminology.getLatest() != null && terminology.getLatest())))
				.count() > 0;
	}
