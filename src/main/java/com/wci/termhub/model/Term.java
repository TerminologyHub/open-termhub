...
public class Term extends AbstractHasModified
	...
	@MultiField(mainField = @Field(type = FieldType.Text), otherFields = {
			...
			@InnerField(suffix = "ngram", type = FieldType.Text, analyzer = "autocomplete_index", searchAnalyzer = "autocomplete_search") })
	private String name;
	...
	@Schema(description = "Concept id of the concept containing this term "
			+ "(typically this is the same as the code, but may be different " + "for some terminologies)")
	...
	public String computeSingleLanguage() {
		...
		return localeMap.keySet().stream().sorted((a, b) -> b.length() - a.length()).findFirst().orElse(null);
	}
