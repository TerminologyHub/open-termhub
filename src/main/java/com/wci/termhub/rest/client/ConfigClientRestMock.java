...
public class ConfigClientRestMock extends RootClientRestMock implements ConfigClient {
	...
	public ConfigClientRestMock() throws Exception {
		...
		List<String> lines = IOUtils
				.readLines(getClass().getClassLoader().getResourceAsStream("mock/config/typeKeyValue.txt"), "UTF-8");
		...
		// Read statements from resources
		lines = IOUtils.readLines(getClass().getClassLoader().getResourceAsStream("mock/config/licenseInfo.txt"),
				"UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
		...
		// Read statements from resources
		lines = IOUtils.readLines(getClass().getClassLoader().getResourceAsStream("mock/config/languageInfo.txt"),
				"UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
		...
		// Read statements from resources
		lines = IOUtils.readLines(getClass().getClassLoader().getResourceAsStream("mock/config/publisherInfo.txt"),
				"UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
		...
		// Read statements from resources
		lines = IOUtils.readLines(getClass().getClassLoader().getResourceAsStream("mock/config/planInfo.txt"), "UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
			}
		...
		// Read statements from resources
		lines = IOUtils.readLines(getClass().getClassLoader().getResourceAsStream("mock/config/resourceInfo.txt"),
				"UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
		...
		// Read statements from resources
		lines = IOUtils.readLines(getClass().getClassLoader().getResourceAsStream("mock/config/terminologyInfo.txt"),
				"UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
	...
	public TerminologyInfo addTerminologyInfo(final TerminologyInfo terminology) throws Exception {
		...
		final Constructor<?> jpaConstructor = terminology.getClass().getConstructor(TerminologyInfo.class);
	...
	public TypeKeyValue addTypeKeyValue(final TypeKeyValue typeKeyValue) throws Exception {
		...
		final Constructor<?> jpaConstructor = typeKeyValue.getClass().getConstructor(TypeKeyValue.class);
	...
	public void enableApiKey(final String apiKey) throws Exception {
		TypeKeyValue tkv = tkvMap.values().stream()
				.filter(t -> "apiKey".equals(t.getType()) && t.getKey().equals(apiKey)).findFirst().orElse(null);
		if (tkv == null) {
	...
	public void deleteApiKey(final String apiKey) throws Exception {
		...
		final ResultListTypeKeyValue list = findTypeKeyValue("type:apiKey AND key:" + StringUtility.escapeQuery(apiKey),
				0, 1, null, null);
