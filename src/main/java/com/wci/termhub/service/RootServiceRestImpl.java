...
public class RootServiceRestImpl {
  ...
  public AuthContext authorize(final HttpServletRequest request) throws Exception {
    ...
    if (!"ADMIN".equals(context.getRole()) && context.getRole() != null
        && !"USER".equals(context.getRole())) {
      throw new RestException(false, 403, "Forbidden", context.getRole());
    }
    return context;
  }
  ...
  public AuthContext authorizeProject(final HttpServletRequest request) throws Exception {
    ...
    if (!"ADMIN".equals(context.getRole()) && context.getRole() != null
        && !"USER".equals(context.getRole()) && !"PROJECT".equals(context.getRole())) {
      throw new RestException(false, 403, "Forbidden", context.getRole());
    }
    return context;
  }
  ...
  // * @throws Exception the exception
  // */
  // public void reindex(final ElasticOperationsService operationsService, final
  // String objects,
  ...
  //
  // for (final Class<? extends HasId> clazz :
  // ModelUtility.getManagedObjects(service, objects)) {
  ...
  // * @throws Exception the exception
  // */
  // public void delete(final ElasticSearchService searchService, final
  ...
  private void adminInProgressCheck() throws Exception {
    if (adminInProgress) {
      throw new RestException(true, 503, "Unable to process request, reindexing in progress.",
          null);
    }
  }
