...
public final class JwtUtility {
	...
	public static void verify(final DecodedJWT djwt, final boolean checkSignature) throws Exception {
		...
		if (userId == null) {
			if (getRole(claims) == null) {
				...
				// block
				logger.error("Non-admin JWT unexpectedly missing ID claim = " + ModelUtility.toJson(claims), 401);
			...
			if (!"admin".equals(userId)
				...
				throw new WebApplicationException("JWT ID claim is unexpectedly not a valid UUID/admin = " + userId,
						401);
			}
		}
	 ...
	 * @throws Exception the exception
	 */
	public static String mockProjectJwt(final String projectId, final String organizationId) throws Exception {
		final Properties prop = PropertyUtility.getProperties();
		final String secret = prop.getProperty("jwt.secret");
		final String audience = getAudience();
		...
		final Builder builder = JWT.create().withAudience(audience).withIssuer(issuer)
				// Later: replace salt with expiration date when we're ready to handle that
				.withClaim(Claims.SALT.getValue(), StringUtility.randomString(8))
				.withClaim(Claims.PROJECT_ID.getValue(), projectId).withClaim(Claims.ORG_ID.getValue(), organizationId)
	 ...
	 * @throws Exception the exception
	 */
	public static String mockJwt(final String userId, final String plan, final String role) throws Exception {
		final Properties prop = PropertyUtility.getProperties();
	 ...
	 * @throws Exception the exception
	 */
	public static String mockJwt(final String userId, final String plan, final String role, final String jwtSecret)
			throws Exception {
	 ...
	 * @throws Exception the exception
	 */
	public static String mockJwt(final String userId, final String plan, final String role, final Integer expiresIn,
	...
	public static String[] getImmutableClaims() {
		return new String[] { "iad", "aud", "iss", "exp" };
	}
	...
	public static AuthResponse proxyAuth(final AuthContext context, final Integer expiresIn,
		...
		if (expiresIn != null && expiresIn > (30 * 24 * 3600)) {
			throw new WebApplicationException("expiresIn is greater than max allowed value = " + expiresIn);
		}
		...
		if (!"ADMIN".equals(context.getRole())) {
			for (final String claim : new String[] { Claims.PLAN.getValue(), Claims.ID.getValue() }) {
				if (lclaims.containsKey(claim)) {
					throw new WebApplicationException("Proxy auth unable to overwrite claim = " + claim, 417);
				}
			}
		...
		if (lclaims.containsKey(Claims.ROLE.getValue())) {
			...
			if (!EnumUtils.isValidEnum(UserRole.class, role)) {
				throw new WebApplicationException("Proxy auth able to overwrite role with illegal value = " + role,
						417);
			}
		}
