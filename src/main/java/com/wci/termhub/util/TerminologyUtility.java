...
public final class TerminologyUtility {
	...
	private static final Map<EclToLuceneConverter.InternalFunction, Pattern> CONVERTER_MAP = getConverterMap();
	...
	private static Map<EclToLuceneConverter.InternalFunction, Pattern> getConverterMap() {
		...
		final EclToLuceneConverter.InternalFunction[] x = EclToLuceneConverter.InternalFunction.values();
	 ...
	 */
	public static Terminology getTerminology(final EntityRepositoryService searchService, final String terminology,
		...
		final ResultList<Terminology> tlist = searchService.find(new SearchParameters(
				"abbreviation:" + StringUtility.escapeQuery(terminology) + " AND publisher:"
						+ StringUtility.escapeQuery(publisher) + " AND version:" + StringUtility.escapeQuery(version),
		...
		if (tlist.getItems().size() > 1) {
			throw new Exception("Too many terminology matches = " + terminology + ", " + publisher + ", " + version);
		}
	 ...
	 * @throws Exception the exception
	 */
	public static Mapset getMapset(final EntityRepositoryService searchService, final String abbreviation,
			final String publisher, final String version) throws Exception {

		final ResultList<Mapset> tlist = searchService.find(new SearchParameters(
				"abbreviation:" + abbreviation + " AND publisher:" + publisher + " AND version:" + version, 2, 0),
				Mapset.class);
		...
		if (tlist.getItems().size() > 1) {
			throw new Exception("Too many terminology matches = " + abbreviation + ", " + publisher + ", " + version);
		}
	 ...
	 * @throws Exception the exception
	 */
	public static Concept getConcept(final EntityRepositoryService searchService, final Terminology terminology,
	 ...
	 * @throws Exception the exception
	 */
	public static Concept getConcept(final EntityRepositoryService searchService, final String terminology,
			final String publisher, final String version, final String code, final String indexName) throws Exception {
		final SearchParameters nameParams = new SearchParameters(2, 0);
		final String t = code.startsWith("V-") ? "SRC" : terminology;
		nameParams.setQuery("code:" + StringUtility.escapeQuery(code) + " AND terminology:" + t + " AND publisher:"
	 ...
	 * @throws Exception the exception
	 */
	public static Concept getConceptRef(final EntityRepositoryService searchService, final Terminology terminology,
			final String code) throws Exception {
	 ...
	 * @throws Exception the exception
	 */
	public static Concept getConceptRef(final EntityRepositoryService searchService, final String terminology,
			final String publisher, final String version, final String code, final String indexName) throws Exception {
		final SearchParameters nameParams = new SearchParameters(2, 0);
		final String t = code.startsWith("V-") ? "SRC" : terminology;
		nameParams.setQuery("code:" + StringUtility.escapeQuery(code) + " AND terminology:" + t + " AND publisher:"
	 ...
	 * @throws Exception the exception
	 */
	public static List<String> getConceptEcl(final EntityRepositoryService searchService, final String terminology,
			final String publisher, final String version, final String code, final String indexName) throws Exception {
		final SearchParameters nameParams = new SearchParameters(2, 0);
		final String t = code.startsWith("V-") ? "SRC" : terminology;
		nameParams.setQuery("code:" + StringUtility.escapeQuery(code) + " AND terminology:" + t + " AND publisher:"
				+ publisher + " AND version:" + version);
		final ResultList<Concept> list = searchService.findFields(nameParams, ModelUtility.asList("ecl"),
				Concept.class);
		if (list.getItems().size() == 0) {
			return null;
		}
	 ...
	 * @throws Exception the exception
	 */
	public static String getConceptName(final EntityRepositoryService searchService, final String terminology,
			final String publisher, final String version, final String code, final String indexName) throws Exception {
		final SearchParameters nameParams = new SearchParameters(2, 0);
		final String t = code.startsWith("V-") ? "SRC" : terminology;
		nameParams.setQuery("code:" + StringUtility.escapeQuery(code) + " AND terminology:" + t + " AND publisher:"
		...
		final ResultList<Concept> list = searchService.findFields(nameParams, ModelUtility.asList("name"),
				Concept.class);
		if (list.getItems().size() == 0) {
			return null;
		}
	...
	public static ConceptTreePosition computeTree(final EntityRepositoryService searchService,
		...
		for (final String code : fullAncPath.split("~")) {
			...
			// finalQuery.append(" AND additionalType:" + treePosition.getAdditionalType());
			// }
	...
	public static List<String> resolveExpression(final EntityRepositoryService searchService,
		...
		final String query = getExpressionQuery(searchService, terminology.getAbbreviation(),
				terminology.getPublisher(), terminology.getVersion(), expression, terminology.getIndexName());
		return searchService.findAllWithFields(query, ModelUtility.asList("code"), Concept.class).stream()
	 ...
	 * @throws Exception the exception
	 */
	public static Set<String> getAncestorCodes(final EntityRepositoryService searchService, final Concept concept)
			throws Exception {
		return concept.getEclClauses().stream().filter(c -> c.matches(EclConceptFieldNames.ANCESTOR + "=[A-Z\\d].*"))
				.map(c -> c.substring(EclConceptFieldNames.ANCESTOR.length() + 1)).collect(Collectors.toSet());
	}
	 ...
	 * @throws Exception the exception
	 */
	public static Set<String> getParentCodes(final EntityRepositoryService searchService, final Concept concept)
			throws Exception {
		return concept.getEclClauses().stream().filter(c -> c.matches(EclConceptFieldNames.PARENT + "=[A-Z\\d].*"))
				.map(c -> c.substring(EclConceptFieldNames.PARENT.length() + 1)).collect(Collectors.toSet());
	}
	...
	public static List<ConceptRef> getParents(final EntityRepositoryService searchService,
		...
		final List<ConceptRelationship> list = searchService.findAll(StringUtility.composeQuery("AND",
				"from.code:" + StringUtility.escapeQuery(concept.getCode()), "hierarchical:true AND active:true"),
	...
	public static void populateConcept(final Concept concept, final IncludeParam includeParam,
		...
		if (concept.getDescendants().isEmpty() && includeParam.isDescendants()) {
			concept.setDescendants(TerminologyUtility.getDescendants(searchService, terminology, concept));
		}
		...
		if (concept.getRelationships().isEmpty() && includeParam.isRelationships()) {
			concept.setRelationships(TerminologyUtility.getRelationships(searchService, terminology, concept));
		}
		...
		if (concept.getTreePositions().isEmpty() && includeParam.isTreePositions()) {
			concept.setTreePositions(TerminologyUtility.getTreePositions(searchService, terminology, concept));
		}
	}
	...
	public static List<ConceptRef> getChildren(final EntityRepositoryService searchService,
		...
		final List<ConceptRelationship> list = searchService.findAll(StringUtility.composeQuery("AND",
				"to.code:" + StringUtility.escapeQuery(concept.getCode()), "hierarchical:true", "active:true"),
				ConceptRelationship.class);
	 ...
	 * @throws Exception the exception
	 */
	public static List<ConceptRelationship> getRelationships(final EntityRepositoryService searchService,
			final Terminology terminology, final Concept concept) throws Exception {

		final SearchParameters params = new SearchParameters(
				StringUtility.composeQuery("AND", "from.code:" + StringUtility.escapeQuery(concept.getCode())), 0, 1000,
				"", true);
	 ...
	 * @throws Exception the exception
	 */
	public static List<ConceptRelationship> getInverseRelationships(final EntityRepositoryService searchService,
			final Terminology terminology, final Concept concept) throws Exception {

		final SearchParameters params = new SearchParameters(
				StringUtility.composeQuery("AND", "to.code:" + StringUtility.escapeQuery(concept.getCode())), 0, 1000,
				"", true);
	 ...
	 * @throws Exception the exception
	 */
	public static List<ConceptTreePosition> getTreePositions(final EntityRepositoryService searchService,
			final Terminology terminology, final Concept concept) throws Exception {
	...
	public static List<ConceptRef> getDescendants(final EntityRepositoryService searchService,
		...
		final String expression = getExpressionQuery(searchService, terminology.getAbbreviation(),
				terminology.getPublisher(), terminology.getVersion(), "<" + concept.getCode(),
	 ...
	 * @throws Exception the exception
	 */
	public static String getExpressionQuery(final EntityRepositoryService searchService, final Terminology terminology,
			final String expression) throws Exception {
		return getExpressionQuery(searchService, terminology.getAbbreviation(), terminology.getPublisher(),
	 ...
	 * @throws Exception the exception
	 */
	public static String getExpressionQuery(final EntityRepositoryService searchService, final String terminology,
			final String publisher, final String version, final String expression, final String indexName)
			throws Exception {
		...
		final String clause = getExpressionClause(searchService, terminology, publisher, version, expression,
				indexName);
		return clause;
	}
	...
	public static String getStyClause(final String expression) {
		...
		// split on "OR" or on ";"
		for (final String tag : expression.trim().split(expression.contains(";") ? ";" : "\\s*OR\\s*")) {
			tags.add(tag.trim());
		}

		final List<String> clauses = tags.stream()
				.map(s -> ((s.startsWith("\"") && s.endsWith("\"") || s.contains(" "))
						? ("semanticTypes:\"" + s.replaceAll("\"", "") + "\"")
						: "semanticTypes:" + s))
	 ...
	 * @throws Exception the exception
	 */
	private static String getExpressionClause(final EntityRepositoryService searchService, final String terminology,
			final String publisher, final String version, final String expr, final String indexName) throws Exception {
	 ...
	 * @throws Exception the exception
	 */
	private static String processInternalFunction(final EntityRepositoryService searchService, final String terminology,
			final String publisher, final String version, final String luceneQuery,
			final EclToLuceneConverter.InternalFunction internalFunction, final String indexName) throws Exception {
		...
		final boolean inlineFlag = internalFunction.name().contains("PARENT")
				|| internalFunction.name().contains("ANCESTOR");
		if (inlineFlag) {
			// Get all ancestors of THIS concept
			final List<String> eclClauses = getConceptEcl(searchService, terminology, publisher, version, code,
					indexName);
			if (eclClauses == null) {
				throw new LocalException("Expression contains missing code = " + terminology + " " + code);
			} else {
				relatedConcepts = eclClauses.stream().filter(e -> e.startsWith(internalFunction.getSearchField() + "="))
		...
		// if not ancestor function
			...
			relatedConcepts = searchService
					.findAllWithFields(
							StringUtility.composeQuery("AND", "terminology:" + terminology, "publisher:" + publisher,
									"version:" + version,
									"ecl:" + internalFunction.getSearchField() + "=" + StringUtility.escapeQuery(code)),
							ModelUtility.asList("code"), Concept.class)
	 ...
	 * @return the string
	 */
	private static String buildOptionsList(final List<String> relatedCodes, final boolean includeIdFieldName) {
...
//	 * @throws Exception the exception
//	 */
//	public static long countConcepts(final EntityRepositoryService<Concept, String> searchService, final String terminology,
...
//				+ " AND version:" + version);
//
//		return searchService.find(params, Concept.class, null, null, ModelUtility.asList(indexName)).getTotal();
//
//	}
...
//	 * @throws Exception the exception
//	 */
//	public static long countRelationships(final EntityRepositoryService<ConceptRelationship, String> searchService,
...
//				+ " AND version:" + version);
//
//		return searchService.find(params, ConceptRelationship.class, null, null, ModelUtility.asList(indexName))
//				.getTotal();
//
//	}
	 ...
	 * @return the array node
	 */
	private static ArrayNode createRelationshipsNode(final Concept concept, final List<Metadata> metadata) {
		...
		for (final ConceptRelationship rel : concept.getRelationships().stream().sorted((a, b) -> {
			...
			// Use same algorithm as ECL to determine for non-SNOMED relationships whether
			// to use
			...
			if (!skipRel && rel.getActive()) {
				...
				if (rel.getTo() != null) {
					...
					if (rel.getTo().getDefined() != null) {
						targetNode.put("definitionStatus", rel.getTo().getDefined() ? "FULLY_DEFINED" : "PRIMITIVE");
					} else {
	...
	public static String fixExpressionQuery(final String query) throws Exception {
		...
		while (true) {
			boolean found = false;
			final Pattern p = Pattern.compile("\\(\\s+\\(([^\\)]+)\\)\\s+(AND|OR|AND NOT)\\s*\\(([^\\)]+)\\)\\s*\\)");
		...
		while (true) {
			...
			final Matcher m = p.matcher(luceneQuery);
			while (m.find()) {
				luceneQuery = luceneQuery.substring(0, m.start()) + "ecl:null=null" + luceneQuery.substring(m.end());
		...
		// Handle "ecl:246075003=(418165002 OR 373545003 )" => "(ecl:246075003=418165002
		// OR
		// ecl:246075003=373545003)"
		// Handle "(288556008 OR 726633004)=725894000" => "(ecl:288556008=725894000 OR
		// ecl:726633004=725894000)"
		// Handle "(288556008 OR 726633004)= ( (407481006 OR 407478001) OR ecl:70296001
		// )"
		// Handle "(288556008 OR 726633004)= ( (407481006 OR 407478001) OR null )"
		// Handle "(288556008 OR 726633004)= ( (407481006 OR 407478001 ) OR (417793006
		// OR 417771001)
		// )"
		...
		// spaces or
		// equals)
		final String rightHandSide = "(\\([^(]+\\)|\\(([^(]*\\([^\\(]+\\)[^(]*)+[^(]*\\)|[^\\s=\\(\\)]+)";

		// require a space before the pattern because this is for attribute restriction
		// it will
		// always
		// have one
		while (true) {
			...
			final Matcher m = p.matcher(luceneQuery);
			while (m.find()) {
				// Either LHS or RHS must have a parens or there is nothing to distribute
				...
				if (left.contains(" AND ") || left.contains(" NOT ")) {
					throw new Exception("Unexpected AND or NOT in left hand side of expression " + left + ", " + query);
				}
				...
				while (m2.find()) {
					...
					// Append from last index to start of this group, then replacement, reset index
					rewrite.append(right.substring(i, m2.start()));
					rewrite.append(repl);
					i = m2.end();
					// right = right.replaceFirst("\\b" + Pattern.quote(m2.group(0)) + "\\b",
					// repl.toString());
					found = true;
				}
				...
				if (found) {
					// logger.info(" GROUP = " + m.start() + ", " + m.end() + ", " + m.group(0));
					// logger.info(" PRE = " + luceneQuery.substring(0, m.start()));
					// logger.info(" REWRITE = " + rewrite);
					// logger.info(" POST = " + luceneQuery.substring(m.end()));
					luceneQuery = luceneQuery.substring(0, m.start()) + " " + rewrite + luceneQuery.substring(m.end());
					// luceneQuery =
	...
	public static Terminology findClosestByReleaseDate(final EntityRepositoryService searchService,
		...
		final ResultList<Terminology> list = searchService
				.find(new SearchParameters("abbreviation:" + StringUtility.escapeQuery(abbreviation) + " AND publisher:"
						+ StringUtility.escapeQuery(publisher), null, null, null, null), Terminology.class);
		Date maxDate = null;
