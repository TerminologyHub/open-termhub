...
public final class TerminologyUtility {
    ...
    public static Terminology getTerminology(final EntityRepositoryService searchService,
        ...
        final ResultList<Terminology> tlist =
                searchService.find(
                        new SearchParameters(
                                getTerminologyAbbrQuery(terminology, publisher, version), 2, 0),
                        Terminology.class);
        ...
        if (tlist.getItems().size() > 1) {
            throw new Exception("Too many terminology matches = " + terminology + ", " + publisher
                    + ", " + version);
        }
    ...
    public static Mapset getMapset(final EntityRepositoryService searchService,
        ...
        final ResultList<Mapset> tlist =
                searchService.find(
                        new SearchParameters(
                                getTerminologyAbbrQuery(abbreviation, publisher, version), 2, 0),
                        Mapset.class);
        ...
        if (tlist.getItems().size() > 1) {
            throw new Exception("Too many terminology matches = " + abbreviation + ", " + publisher
                    + ", " + version);
        }
    ...
    public static Concept getConceptRef(final EntityRepositoryService searchService,
        final Terminology terminology, final String code) throws Exception {
        return getConceptRef(searchService, terminology.getAbbreviation(),
                terminology.getPublisher(), terminology.getVersion(), code);
    }
    ...
    public static Concept getConceptRef(final EntityRepositoryService searchService,
        ...
        final ResultList<Concept> list =
                searchService.findFields(nameParams, ModelUtility.asList("id", "name", "code",
                        "terminology", "version", "publisher", "leaf"), Concept.class);
        if (list.getItems().isEmpty()) {
            return null;
        }
        return list.getItems().get(0);
    ...
    public static ConceptTreePosition computeTree(final EntityRepositoryService searchService,
        ...
        for (final String code : fullAncPath.split("~")) {
            ...
            finalQuery.append("concept.code:" + QueryParserBase.escape(code))
                    ...
                    .append(" AND version:")
                    .append(StringUtility.escapeQuery(treePosition.getVersion()))
            ...
            if (list.getItems().isEmpty()) {
                throw new Exception(
                        "Unable to find matching tree position for ancestor = " + finalQuery);
            }
    ...
    public static void populateConcept(final Concept concept, final IncludeParam includeParam,
        ...
        if (concept.getChildren().isEmpty() && includeParam.isChildren()) {
            concept.setChildren(
                    TerminologyUtility.getChildren(searchService, terminology, concept));
        }
        if (concept.getDescendants().isEmpty() && includeParam.isDescendants()) {
            concept.setDescendants(
                    TerminologyUtility.getDescendants(searchService, terminology, concept));
        }
        ...
        if (concept.getInverseRelationships().isEmpty() && includeParam.isInverseRelationships()) {
            concept.setInverseRelationships(TerminologyUtility
                    .getInverseRelationships(searchService, terminology, concept));
        }
    ...
    public static List<ConceptRef> getDescendants(final EntityRepositoryService searchService,
        ...
        while (true) {
            final List<Concept> concepts = searchService
                    .findFields(params, IncludeParam.getConceptBaseFields(), Concept.class)
                    .getItems();
            if (concepts.isEmpty()) {
                break;
            }
    ...
    public static boolean isStyExpression(final String expr) {
        for (final String part : expr.split("( OR |;)")) {
            ...
            if (part.matches(".*[A-Z][A-Z].*")
                    || !part.matches("^\\s*[a-zA-Z][a-z][a-zA-Z\\/ ,\\d;]+")) {
                return false;
            }
    ...
    public static String toDiagramModel(final Concept concept, final List<Metadata> metadata) {
        ...
        conceptNode.put("definitionStatus", (concept.getDefined() != null && concept.getDefined())
                ? "FULLY_DEFINIED" : "PRIMITIVE");
        ...
        for (final ConceptRelationship rel : concept.getRelationships().stream().sorted((a, b) -> {
            ...
            if (!skipRel && rel.getActive()) {
                final String type = ((rel.getHierarchical() != null && rel.getHierarchical())
                        || rel.getAdditionalType().equals("isa")) ? "116680003"
                                : rel.getAdditionalType();
    ...
    public static String fixExpressionQuery(final String query) throws Exception {
        ...
        while (true) {
            ...
            while (m.find()) {
                ...
                if (second.contains(" AND ") || second.contains(" NOT ")
                        || second.contains("ecl:")) {
                    continue;
                }
                found = true;
                ...
                repl.append(" )");
                luceneQuery =
                        luceneQuery.substring(0, m.start()) + repl + luceneQuery.substring(m.end());
        ...
        while (true) {
            ...
            final Matcher m = p.matcher(luceneQuery);
            while (m.find()) {
                luceneQuery = luceneQuery.substring(0, m.start()) + "ecl:null=null"
        ...
        while (true) {
            ...
            while (m.find()) {
                ...
                if (left.contains(" AND ") || left.contains(" NOT ")) {
                    throw new Exception("Unexpected AND or NOT in left hand side of expression "
                            + left + ", " + query);
                }
                ...
                if (found) {
                    ...
                    luceneQuery = luceneQuery.substring(0, m.start()) + " " + rewrite
                            + luceneQuery.substring(m.end());
                    // luceneQuery =
    ...
    public static Terminology findClosestByReleaseDate(final EntityRepositoryService searchService,
        ...
        final ResultList<Terminology> list =
                searchService.find(new SearchParameters(
                        "abbreviation:" + StringUtility.escapeQuery(abbreviation)
                                + " AND publisher:\"" + StringUtility.escapeQuery(publisher) + "\"",
                        null, null, null, null), Terminology.class);
        Date maxDate = null;
