...
public final class TestUtility {
	...
	public static User getMockUser(final String value) throws Exception {
		final List<String> lines = IOUtils
				.readLines(TestUtility.class.getClassLoader().getResourceAsStream("mock/user/users.txt"), "UTF-8");
	...
	public static Concept getMockConcept(final String conceptName) throws Exception {
		final InputStream is = Application.class.getClassLoader().getResourceAsStream("mock/terminology/concepts.txt");
		final List<Concept> concepts = ModelUtility.fromJson(IOUtils.toString(is, "UTF-8"),
	...
	public static LicenseInfo getMockLicense(final String license) throws Exception {
		final List<String> lines = IOUtils.readLines(
				TestUtility.class.getClassLoader().getResourceAsStream("mock/config/licenseInfo.txt"), "UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
			}
	...
	public static FamilyInfo getMockFamily(final String family) throws Exception {
		final List<String> lines = IOUtils.readLines(
				TestUtility.class.getClassLoader().getResourceAsStream("mock/config/familyInfo.txt"), "UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
			}
	...
	public static PublisherInfo getMockPublisher(final String publisher) throws Exception {
		final List<String> lines = IOUtils.readLines(
				TestUtility.class.getClassLoader().getResourceAsStream("mock/config/publisherInfo.txt"), "UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
			}
	...
	public static PlanInfo getMockPlan(final String plan) throws Exception {
		final List<String> lines = IOUtils
				.readLines(TestUtility.class.getClassLoader().getResourceAsStream("mock/config/planInfo.txt"), "UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
			}
	...
	public static ResourceInfo getMockResource(final String resource) throws Exception {
		final List<String> lines = IOUtils.readLines(
				TestUtility.class.getClassLoader().getResourceAsStream("mock/config/resourceInfo.txt"), "UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
			}
	...
	public static TerminologyInfo getMockTerminology(final String terminology) throws Exception {
		final List<String> lines = IOUtils.readLines(
				TestUtility.class.getClassLoader().getResourceAsStream("mock/config/terminologyInfo.txt"), "UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
			}
	...
	public static TypeKeyValue getMockTypeKeyValue(final String typeKeyValue) throws Exception {
		final List<String> lines = IOUtils.readLines(
				TestUtility.class.getClassLoader().getResourceAsStream("mock/config/typeKeyValue.txt"), "UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
			}
	...
	public static LanguageInfo getMockLanguage(final String language) throws Exception {
		final List<String> lines = IOUtils.readLines(
				TestUtility.class.getClassLoader().getResourceAsStream("mock/config/languageInfo.txt"), "UTF-8");
		for (final String line : lines) {
			if (line.isEmpty() || line.startsWith("#")) {
				continue;
			}
