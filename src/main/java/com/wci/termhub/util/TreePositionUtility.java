...
public final class TreePositionUtility {
	 ...
	 * @throws Exception the exception
	 */
	public static boolean computeRelationshipClauses(final Map<String, Metadata> relaMap, final String terminology,
			final String type, final String toCode, final String toValue, final Set<String> clauses) throws Exception {
		boolean flag = false;
	...
	/**
	 * Exclude these standard non-defining types for performance (and they are HIGH
	 * volume). NOTE: keep LOINC "answer_to" and some others
	 * 
	 * @param type the type
	 * @return true, if successful
	 */
	public static boolean excludeRelationshipFromEcl(final String type) {
		...
		return StringUtility.isEmpty(type) || type.endsWith("_to") || type.endsWith("_of") || type.endsWith("Of")
				|| type.endsWith("_by") || type.endsWith("_for") || type.endsWith("_in") || type.startsWith("inverse_");
	}
	...
	public static List<String> computeTransitiveClosure(final String code, final String ancestorPath,
		...
		// If not leaf node, compute children and add results that are not null
			...
			for (final String chd : parChd.get(code)) {
				final List<String> chdResults = computeTransitiveClosure(chd, conceptPath, parChd, cycleTolerant);
				if (chdResults != null) {
