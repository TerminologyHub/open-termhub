...
public class ValueSetProviderR4UnitTest extends AbstractFhirR4ServerTest {
    ...
    public void testConcurrentAdd() throws Exception {
        ...
        Callable<MethodOutcome> writeTask1 = () -> {
            return createValueSet(vs1);
        };

        Callable<MethodOutcome> writeTask2 = () -> {
            return createValueSet(vs2);
        };

        Future<MethodOutcome> future1 = executor.submit(writeTask1);
        // Ensure the first write starts before the second
        Thread.sleep(10); // Small delay to increase chance of overlap

        Future<MethodOutcome> future2 = executor.submit(writeTask2);

        executor.shutdown();

        MethodOutcome created1 = future1.get();
        MethodOutcome created2 = future2.get();

    // Exactly one should fail
    Assertions.assertTrue(created1.getCreated() ^ created2.getCreated(), "Exactly one write should fail due to locking");
    ...
    private MethodOutcome createValueSet(ValueSet vs) throws Exception {
        try {
