...
class BulkLoaderServiceImplTest extends AbstractServerTest {
     ...
     */
    @Test
    void testDoCodeSystemLoad_Success() throws Exception {
        // 1. Arrange
        File codeSystemFile = codeSystemResource.getFile();
        // This file will be deleted by the service, so copy it to a temp location
        File tempTestFile = new File(tempDir, codeSystemFile.getName());
        updateVersionAndCopy(tempTestFile);

        // 2. Act
        CodeSystem result = bulkLoaderService.doCodeSystemLoad(tempTestFile);
     ...
     */
    @Test
    void testStartAsyncCodeSystemLoad_Success() throws Exception {
        // 1. Arrange
        ...
        Map<String, List<String>> processResultMap = new ConcurrentHashMap<>();
        Map<String, Long> processProgressMap = new ConcurrentHashMap<>();
        Map<String, Exception> processExceptionMap = new ConcurrentHashMap<>();
        ProgressListener mockListener = Mockito.mock(ProgressListener.class);

        // 2. Act
        bulkLoaderService.startAsyncCodeSystemLoad(
                processId, tempTestFile, mockListener, processResultMap, processProgressMap, processExceptionMap);
        ...
        await()
                ...
                .until(() -> processResultMap.containsKey(processId));

        // Verify the result map was updated correctly
        assertThat(processResultMap).containsKey(processId);
        assertTrue(processResultMap.get(processId) != null && !processResultMap.get(processId).isEmpty());
     ...
     */
    @Test
    void testStartAsyncCodeSystemLoad_Failure() throws Exception {
        // 1. Arrange
        String processId = "process-456";
        File testFile = new File(tempDir, "invalid-codesystem.json");
        String invalidJson = "{\"resourceType\": \"CodeSystem\", \"id\": \"invalid-cs\",,}"; // Malformed JSON
        ...
        // 2. Act
        bulkLoaderService.startAsyncCodeSystemLoad(
                processId, testFile, mockListener, processResultMap, processProgressMap, processExceptionMap);

        // 3. Assert
        // Use Awaitility to wait for the async method to fail and update the exception map.
        await()
                .atMost(Duration.ofSeconds(10))
                .until(() -> processExceptionMap.containsKey(processId));
    ...
    void testConcurrentAsyncAndSyncLoad() throws Exception {
        // 1. Arrange
        String processId = "process-789";
        // 1. Arrange
        File codeSystemFile = codeSystemResource.getFile();
        // This file will be deleted by the service, so copy it to a temp location
        File tempTestFile1 = new File(tempDir, "async-" + codeSystemFile.getName());
        updateVersionAndCopy(tempTestFile1);
        File tempTestFile2 = new File(tempDir, "sync-" + codeSystemFile.getName());
    ...
    private void updateVersionAndCopy(File tempTestFile) throws IOException {
        ObjectMapper objectMapper = new ObjectMapper();
        String originalJsonContent = FileUtils.readFileToString(codeSystemResource.getFile(), StandardCharsets.UTF_8);
        ObjectNode rootNode = (ObjectNode) objectMapper.readTree(originalJsonContent);
