...
public class ProxyTester {
	...
	/** The proxy map. */
	private final Map<Class<?>, Map<Integer, Object>> proxyMap = new HashMap<>();

	/** The field proxy map. */
	private final Map<String, Map<Integer, Object>> fieldProxyMap = new HashMap<>();
	 ...
	 * under test, and will instead simply test all the fields in the include list.
	 * 
	...
	protected Object makeProxy(final String fieldName, final Class<?> type, final int initializer) throws Exception {
		...
		if (type == Float.class || type == float.class) {
			return Double.valueOf((initializer * 1.0) / 100).floatValue();
		}
		...
		if (type.isInterface()) {
			return Proxy.newProxyInstance(type.getClassLoader(), new Class[] { type }, new DummyInvocationHandler());
		}
		...
		Object callback;
		callback = Proxy.newProxyInstance(callbackClass.getClassLoader(), new Class[] { fixedValueClass },
				new DummyInvocationHandler());

		final Method createMethod = enhancerClass.getMethod("create", new Class[] { Class.class, callbackClass });
